...
NoteTrace uses [svelte-i18n](https://github.com/kaisermann/svelte-i18n) with one JSON file per locale in `src/i18n/`. The English file at `src/i18n/en.json` is the source of truth.

### Two ways to contribute (pick either)

**A. Weblate (preferred, no code required).** NoteTrace is hosted at [hosted.weblate.org/projects/notetrace/](https://hosted.weblate.org/projects/notetrace/). Pick a language, translate strings in the web UI, and pull requests to `dev` are opened automatically. If your language isn't in the list, click **Add new translation** and it'll be approved. If you already have a completed JSON file from prior work, seed the language with **Files → Upload translation → Add as translation** on the language page.
...
- **Match the tone.** NoteTrace's English copy is informal and direct ("Take a note", "Tidy a Note"). Try to keep that register rather than translating to a more formal style.
...
- **Do not translate proper nouns or product names.** `NoteTrace`, `Trace` (the AI assistant), `Google Keep`, `Markdown` stay as-is.
...
<input placeholder={$_('routes.notes.search_placeholder')} />
```
...
Every user-facing string in the app is expected to be a key in `en.json`, added in the same commit as the UI that uses it (see instrumenting guidance above); hardcoded English literals get flagged in review. `npm run i18n:check` runs against every locale file to catch missing translations and unresolved keys.
