<script>
  ...
  // Wizard variants:
  //   1. Native local mode: skip user-management step, just collect display name
  //   2. PWA with no users yet (setup_required): force admin account creation
  //   3. PWA with users already present: show multi-user toggle
  ...
  // moment of closure instead of just snapping to /notes. Honors
...
<div class="wizard">
  {#if wizardDone}
    <!-- Celebration screen — brief beat between finishing the wizard and
         landing on /notes, so onboarding has a moment of closure
    ...
    <div class="wizard-done">
      ...
      <p class="wizard-done-sub">Welcome to NoteTrace. Loading your notes…</p>
    </div>
  {:else}
  <div class="progress">
  ...
  {#key step}
    <div class="step" in:fly={{ x: dir > 0 ? 60 : -60, duration: 280, easing: cubicOut }}>
      ...
      {#if STEPS[step] === 'welcome'}
        <div class="step-hero">
          <img class="logo-icon" src={iconUrl('/icons/logo.png')} alt="" />
          <h1 class="step-title">{$_('wizard_ct.welcome')}</h1>
          <p class="step-desc">
            Trace Every Thought.
            Self-hosted notes, lists, and reminders.
          </p>
        </div>
      ...
      {:else if STEPS[step] === 'account'}
        {#if _forceAccountCreation}
          <div class="step-hero compact">
            ...
            <p class="step-desc">Set up an admin account to secure your NoteTrace instance. You can invite household members later.</p>
          </div>
        {:else}
          <div class="step-hero compact">
            <span class="material-symbols-rounded hero-icon">group</span>
            <h1 class="step-title">Multi-user setup</h1>
            <p class="step-desc">NoteTrace can run in single-user mode (default) or multi-user mode with separate logins. You can always enable this later in Settings.</p>
          </div>

          <div class="toggle-row">
            <div>
              <div class="toggle-label">{$_('wizard_ct.enable_user_accounts')}</div>
              <div class="toggle-hint">Each user gets their own notes, settings, and profile</div>
            </div>
      ...
      {:else if STEPS[step] === 'done'}
        ...
        <div class="step-actions">
          <button class="btn-primary" on:click={finish}>{$_('wizard_ct.open_notetrace')}</button>
        </div>
      {/if}
    </div>
  {/key}
