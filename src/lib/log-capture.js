 ...
 *   - Toggle via Settings. Persists in localStorage as `note:verboseLogging`.
 ...
 *     `logs/notetrace-YYYY-MM-DD.log`. User exports the file via
...
function _buildHeader() {
  ...
  try {
    if (_isNative) {
      const url = localStorage.getItem('note:lastServerUrl') || '';
  ...
  const lines = [
    `NoteTrace ${_appVersion} · ${mode} · ${new Date().toISOString()}`,
  ];
...
// ── State ──────────────────────────────────────────────────────────────────
let _verbose = (() => {
  try { return localStorage.getItem('note:verboseLogging') === '1'; } catch { return false; }
})();
...
function _todayLogPath() {
  return `${LOG_DIR}/notetrace-${new Date().toISOString().slice(0, 10)}.log`;
}
...
async function _writeCrashReport(detail) {
  ...
  try {
    ...
    try { localStorage.setItem('note:lastCrashFile', path); } catch {}
    _pruneOldCrashFiles();
  } catch {}
}
...
export function setVerboseLogging(on) {
  _verbose = !!on;
  try {
    if (on) localStorage.setItem('note:verboseLogging', '1');
    else    localStorage.removeItem('note:verboseLogging');
  } catch {}
...
export async function getLastCrashFileUri() {
  if (!_isNative) return null;
  try {
    const path = localStorage.getItem('note:lastCrashFile');
    if (!path) return null;
    const { Filesystem, Directory } = await import('@capacitor/filesystem');
    // Verify file still exists.
    try { await Filesystem.stat({ path, directory: Directory.Data }); }
    catch { localStorage.removeItem('note:lastCrashFile'); return null; }
    const { uri } = await Filesystem.getUri({ path, directory: Directory.Data });
    return { uri, path };
  } catch {
    return null;
...
export function hasCrashReport() {
  try { return !!localStorage.getItem('note:lastCrashFile'); } catch { return false; }
}

/** Forget the latest crash report after the user shares or dismisses it. */
export function clearCrashReport() {
  try { localStorage.removeItem('note:lastCrashFile'); } catch {}
}
