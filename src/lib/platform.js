...
/**
 * Server-injected base path (when NoteTrace is mounted at a subpath via
 * BASE_URL env var, e.g. `/notetrace`). Empty string when running at root,
 * or in native where API calls go through getServerUrl() which already
 * carries any path component the user configured.
 */
const _basePath = (typeof window !== 'undefined' && window.__NOTE_CONFIG__ && window.__NOTE_CONFIG__.basePath) || '';
...
export function getNativeMode() {
  ...
  return localStorage.getItem('note:nativeMode') || null;
}

export function setNativeMode(mode) {
  if (mode) {
    localStorage.setItem('note:nativeMode', mode);
  } else {
    localStorage.removeItem('note:nativeMode');
  }
}
...
export function getServerUrl() {
  ...
  return localStorage.getItem('note:serverUrl') || null;
}
...
export function setServerUrl(url) {
  if (url) {
    ...
    localStorage.setItem('note:serverUrl', clean);
    // Keep a copy for image cache lookups even after disconnecting
    localStorage.setItem('note:lastServerUrl', clean);
  } else {
    localStorage.removeItem('note:serverUrl');
...
export function setAuthToken(token) {
  if (token) localStorage.setItem('note:authToken', token);
  else localStorage.removeItem('note:authToken');
}
...
// https://app.notetrace.local after the hostname flip for password-
...
export function resolveAssetUrl(path) {
  ...
  if (isNative) {
    ...
    const url = getServerUrl() || localStorage.getItem('note:lastServerUrl') || '';
    if (url) {
...
export function apiUrl(path) {
  if (isNative) {
    ...
    return path; // native local — caller redirects to NoteApiNative
  }
  return _basePath + path;
}
