 ...
 *     version: 1,
 *     created_at: 'ISO',
 *     tables: { notes: [...rows], user_settings: [...], ... },
 *     settings: { key: value, ... },
...
export async function importLocalZip(zipBlob) {
  ...
  if (!dbFile) throw new Error('Not a NoteTrace local backup ZIP (missing database.json)');
  const dbText = await dbFile.async('string');
  const snapshot = JSON.parse(dbText);
  if (snapshot.format !== 'notetrace-local-snapshot') {
...
export async function importLocalSnapshot(snapshot) {
  ...
  if (!snapshot || snapshot.format !== 'notetrace-local-snapshot') {
