...
export async function loadAuthState() {
  ...
  if (isNative && getServerUrl()) {
    const cached = localStorage.getItem('note:cachedUser');
    if (cached) {
      try {
...
async function _fetchAuthFromServer() {
  try {
    ...
    if (user) {
      ...
      localStorage.setItem('note:cachedUserMgmt', active ? '1' : '0');
    }
...
export async function logout() {
  ...
  // the server to use the notetrace://oidc-callback deep link as
  ...
  try {
    ...
    if (isNative) {
      try {
        const raw = localStorage.getItem('note:oidc_logout_hint');
