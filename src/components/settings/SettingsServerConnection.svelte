...
<script>
  ...
  async function pushAll() {
    const ok = await confirmDialog({
      ...
      message: 'Marks every note as pending and sends it to the server. Use this once when first connecting a server to an existing local-mode library.',
    ...
    {:else}
      ...
      <div class="row state-row">
        ...
        <div class="state-body">
          ...
          <div class="state-sub">All data stays on this device. Connect to a NoteTrace server below to sync.</div>
        </div>
      </div>
      ...
      {#if localCounts && localCounts.total > 0}
        <div class="merge-counts">
          ...
          <div class="merge-counts-grid">
            {#if localCounts.notes    > 0}<div><strong>{localCounts.notes}</strong> {localCounts.notes === 1 ? 'note' : 'notes'}</div>{/if}
  ...
  <div class="merge-overlay" use:portal use:closeOnBack={() => {}} transition:fade={{ duration: 150 }}>
    <div class="merge-dialog">
      ...
      <div class="merge-counts">
        ...
        <div class="merge-counts-grid">
          {#if migrationSummary.success.notes    > 0}<div><strong>{migrationSummary.success.notes}</strong> {migrationSummary.success.notes === 1 ? 'note' : 'notes'}</div>{/if}
