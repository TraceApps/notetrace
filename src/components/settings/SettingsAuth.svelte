<script>
  ...
  //
  //   NoteTrace-specific: this code lives in SettingsAuth.svelte (own
  ...
  const PROVIDER_PRESETS = [
    {
      ...
      help: 'Auth0 adds custom claims under a namespaced URL like "https://notetrace.app/roles" — leave the admin claim blank for now and contact your tenant admin to set up a rule that exposes role membership.',
      hides: [],
    },
    {
      id: 'authelia',
  ...
  function _defaultRedirectUri() {
    ...
    const basePath = window.__NOTE_CONFIG__?.basePath || '';
    ...
    <div class="card settings-card">
      <div class="setting-row">
        <div>
          ...
          <div class="setting-desc">Only admins can configure identity providers. If you need this access, ask the admin of this NoteTrace instance to promote your account.</div>
        </div>
      </div>
    </div>
  {:else}
          ...
          <div class="oidc-form" transition:slide={{ duration: 180 }}>
            ...
            <div class="setting-row" style="padding:0">
              <div>
                ...
                <div class="setting-desc">Allow anyone with an account at this IdP to create a brand-new NoteTrace account on first sign-in. OFF = admin must invite first. Leave OFF for shared IdPs (Google, work SSO) unless you actually want blanket onboarding.</div>
              </div>
