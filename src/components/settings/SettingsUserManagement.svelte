<script>
  ...
  async function loadPendingInvites() {
    try { pendingInvites = await NoteApi.get('/api/auth/invites'); }
  ...
  async function revokeInvite(token) {
    ...
    try {
      await NoteApi.del(`/api/auth/invites/${token}`);
  ...
  async function loadUsers() {
    try {
      umUsers = await NoteApi.get('/api/auth/users');
  ...
  async function deleteUser(u) {
    ...
    try {
      await NoteApi.del(`/api/auth/users/${u.id}`);
      await loadUsers();
  ...
  async function disableUserManagement() {
    ...
    try {
      ...
      await NoteApi.del('/api/auth/management');
