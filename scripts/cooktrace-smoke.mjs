...
ok(r.status === 200 && list.length === 6 && list.at(-1).checked === true, `lists the CookTrace shopping list, checked last (${list.map(i => i.name).join(', ')})`);
...
ok(got.find(i => i.id === limes.id).checked === true, 'CookTrace has it checked');
