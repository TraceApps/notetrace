 ...
 * outgoing webhooks and (via its pure classifier functions) by
 * image-localizer.js's own resolve-then-check flow. Pure functions plus
 * one DNS lookup against 'localhost' (resolved via the OS hosts file,
 * no external network needed), no db.js import, runs without a
 * compiled better-sqlite3 native binding.
 */
...
test('isPrivateOrLoopback blocks loopback, RFC1918, CGNAT, and IPv6 ULA', () => {
  assert.equal(isPrivateOrLoopback('0.0.0.0'), true);
  ...
  assert.equal(isPrivateOrLoopback('100.64.0.1'), true);  // CGNAT, image-localizer.js's own pre-existing check
  assert.equal(isPrivateOrLoopback('100.63.255.255'), false); // just outside the CGNAT range
