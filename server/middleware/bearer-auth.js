 ...
 *
 * Bearer-token auth for personal-access-token consumers (currently just
 * /api/mcp). Validates the Authorization header against api_tokens,
 * attaches { user, token } to the request, and rate-limits per token.
 */
 ...
 *   router.get('/foo', bearerAuth, requireScope('read:foo'), handler)
 */
