  ...
  try {
    ...
    if (svc === 'gotify') {
      ...
      resp = await fetch(`${url.replace(/\/+$/, '')}/message?token=${encodeURIComponent(token)}`, {
        ...
        body: JSON.stringify({ title: title || 'NoteTrace', message: message || 'Test notification — connected!', priority: priority || 5 }),
      });
    } else if (svc === 'ntfy') {
      ...
      const headers = { 'Title': title || 'NoteTrace', 'Priority': String(Math.min(5, priority || 5)) };
      ...
      const body = { title: title || 'NoteTrace', body: message || 'Test notification — connected!', type: 'info' };
      if (tag) body.tag = tag;
