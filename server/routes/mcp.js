 ...
 * default, so no existing user sees any change. Write tools
 ...
 * require MCP_DESTROY_ENABLED=1 AND `mcp:destroy` scope AND every call
 * to include confirm=true. Any missing gate and the corresponding tools
...
router.get('/', (req, res) => {
  res.status(405).json({
    ...
    error: {
      code: -32000,
      message: 'GET not supported: NoteTrace runs MCP in stateless mode. Use POST.',
    },
  });
});
