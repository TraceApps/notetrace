...
const LOGOUT_COOKIE = 'note_oidc_logout';
 ...
 * Validates state/nonce, exchanges code for tokens, mints note_token, redirects.
 */
router.get('/callback/:providerId', wrap(async (req, res) => {
  ...
  try {
    ...
    if (stored.mobile) return res.redirect(`notetrace://oidc-callback/?error=callback_failed`);
  ...
  if (stored.linkUserId) {
    try {
      ...
      if (stored.mobile) return res.redirect(`notetrace://oidc-callback/?error=${msg}`);
      return _redirectToLogin(res, stored.returnPath, msg);
    }
    if (stored.mobile) return res.redirect(`notetrace://oidc-callback/?linked=1`);
  ...
  // Mint identical note_token to password path
  ...
  if (stored.mobile) {
    ...
    // and Android routes the launch intent to NoteTrace's appUrlOpen
    ...
    let deepLink = `notetrace://oidc-callback/?token=${encodeURIComponent(token)}`;
  ...
  res.cookie('note_token', token, { ...COOKIE_OPTS, maxAge: sessionMaxAge() });
 ...
 * Clears NoteTrace's own session, then (when we recorded a provider at
