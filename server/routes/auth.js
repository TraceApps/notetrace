...
router.post('/login', rateLimitLogin, wrap((req, res) => {
  ...
  res.cookie('note_token', token, cookieOpts);
...
router.post('/register', wrap((req, res) => {
  ...
  // existed (user_id IS NULL). Stays scoped to NoteTrace's own tables.
  if (isFirst) {
    claimAnonymousData(user.id);
    res.cookie('note_token', signToken(user), COOKIE_OPTS);
  }
...
router.put('/password', requireAuth, wrap((req, res) => {
  ...
  res.cookie('note_token', signToken(updated), { ...COOKIE_OPTS, maxAge: sessionMaxAge() });
  res.json({ ok: true });
}));
...
router.get('/users/list', requireAuth, wrap((req, res) => {
  // Available in multi-user mode, for the note sharing picker. Single-user
  // instances (no user management) get [] since there are no peers.
...
// sharing grants reference. Password is a separate
