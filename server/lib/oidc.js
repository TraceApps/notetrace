 ...
 *
 * Wraps the `openid-client` library with NoteTrace-specific concerns:
...
/**
 * Map OIDC claims to a NoteTrace user. Implements the resolved
...
export function resolveUser(provider, claims) {
  ...
  //   auto_register_new_users — auto-create a NoteTrace account for first-time
  ...
  if (email) {
    ...
    if (collision && !autoLink) {
      throw new Error(`Your ${provider.display_name || 'OIDC'} account matches an existing NoteTrace user. Sign in once with your password and link this provider from Profile → Linked accounts. After that, SSO will sign you in directly.`);
    }
  }
  ...
  // 4. Reject
  throw new Error('No NoteTrace account is linked to this identity. Ask your admin to enable Auto-register new users on this provider, or have them invite you first.');
}
...
export function linkUser(userId, providerId, sub, emailVerified) {
  ...
  if (existing && existing.user_id !== userId) {
    throw new Error('This provider identity is already linked to a different NoteTrace account.');
  }
