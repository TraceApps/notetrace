 ...
 *
 * Outgoing webhooks. When a subscribed event happens (a note is
 * created, a checklist is fully checked off, a reminder fires), fire a signed HTTP POST to each of the user's enabled webhooks
 ...
 * user waiting for a real note event. Bypasses
...
export async function sendTestWebhook({ userId, id }) {
  ...
  await _deliverWithRetry(row, 'test', { message: 'This is a test event from NoteTrace.' });
