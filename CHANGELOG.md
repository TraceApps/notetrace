...
### Added
- **Put a whole checklist back with one tap** ([#7](https://github.com/TraceApps/notetrace/issues/7), requested by @bonsairobo). A shopping list is the same list next week, and starting it again meant tapping every item you had ticked off. The bar that counts what is checked now offers **Uncheck All**, and they all return to the list at once. It works with no connection and waits with everything else, and a repeating task keeps its date rather than being moved on, since that only happens when you tick one off.
